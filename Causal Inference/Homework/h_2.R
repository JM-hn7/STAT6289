...
trimmed.dat = cbind(trimmed.dat,lambda_ht,lambda_strat)
m1 = lm(BirthWeight~., data = trimmed.dat[, c(1:12,14)])
m2 = lm(BirthWeight~., data = trimmed.dat[, c(1:12,14,17)])
m3 = lm(BirthWeight~., data = trimmed.dat[, c(1:12,14,18)])
anova(m1,m2)
anova(m1,m3)


#m
terms = data.frame(block = 1:7, bias_ht = 0, var_ht = 0, biasvar_ht = 0
                   , bias_sub = 0, var_sub = 0, biasvar_sub = 0)
coef = as.matrix(coefficients(m1)[2:13])

for (i in 1:7) {
  df_current = trimmed.dat[block==i,]
  df_current_m = df_current[, c(1:12,17)]
  
  for (j in 1:12) {
  df_current_m[,j] = df_current_m[,j]*df_current_m[,13]  
  }
  
  sig2 = var(df_current$BirthWeight)
  N1 = nrow(df_current)[1]
  
  mu = (as.matrix(rbind(colSums(df_current_m[df_current$Smokes == 0, 1:12])/
                          sum(df_current_m[df_current$Smokes ==0,13]), 
                        colSums(df_current_m[df_current$Smokes ==1, 1:12])/
                          sum(df_current_m[df_current$Smokes == 1, 13]))) %*%
          coef + coefficients(m1)[1])/N1
  terms$bias_ht[i] = mu[2,1] - mu[1,1]
  terms$var_ht[i] = (sig2/N1^2)*sum(df_current$lambda_ht^2)
  terms$biasvar_ht[i] = terms$var_ht[i] + terms$bias_ht[i]^2
  
  
  
  
  df_current_s = df_current[,c(1:12, 18)]
  
  for (j in 1:12) {
    df_current_s[,j] = df_current_s[,j]*df_current_s[,13]
  }
  
  
  mu1 = (as.matrix(rbind(colSums(df_current_s[df_current$Smokes == 0, 1:12])/
                          sum(df_current_s[df_current$Smokes ==0,13]), 
                        colSums(df_current_s[df_current$Smokes ==1, 1:12])/
                          sum(df_current_s[df_current$Smokes == 1, 13]))) %*%
          coef + coefficients(m1)[1])/N1
  
  terms$bias_sub[i] = mu1[2,1] - mu1[1,1]
  terms$var_sub[i] = (sig2/N1^2)*sum(df_current$lambda_strat^2)
  terms$biasvar_sub[i] = terms$var_sub[i] + terms$bias_sub[i]^2
